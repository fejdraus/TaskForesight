...
│   ├── TaskForesight.Server/          # ASP.NET Core хост (API + Hangfire)
│   ├── TaskForesight.Shared/          # DTO, интерфейсы, общие модели
│   ├── TaskForesight.Core/            # Бизнес-логика (collector, processor, analytics)
│   └── TaskForesight.McpServer/       # MCP сервер для AI (отдельный console app)
├── tests/
...
- [ ] Создать репозиторий TaskForesight
- [ ] dotnet new sln + 5 проектов (Client, Server, Shared, Core, McpServer)
- [ ] Docker: PostgreSQL 16 + AGE + pgvector
- [ ] init.sql: расширения, граф, таблицы, индексы
- [ ] MudBlazor + InteractiveAuto setup (по паттерну CreatioDownloader3)
- [ ] appsettings.json + IOptions
- [ ] Swagger на Server

### Этап 2: Shared + Core — модели и коллектор (3 дня)
- [ ] DTO в Shared (все records)
- [ ] IAnalyticsDataService интерфейс
- [ ] JiraClient (HttpClient + Basic Auth + Polly)
- [ ] BatchCollector (IAsyncEnumerable, pagination)
- [ ] LinkResolver (рекурсивный обход, пост-релизные баги)
- [ ] ChangelogParser

### Этап 3: Core — процессор (3 дня)
- [ ] TimeCalculator, ReturnDetector, CostCalculator
- [ ] TaskClassifier
- [ ] EmbeddingGenerator (OpenAI API)
- [ ] GraphBuilder (AGE Cypher)
- [ ] DataProcessor (оркестратор)

### Этап 4: Core — storage + analytics (3 дня)
- [ ] TaskRepository (Npgsql + Dapper)
- [ ] VectorRepository (pgvector)
- [ ] GraphRepository (Konnektr.Npgsql.Age: CreateCypherCommand + параметры)
- [ ] EstimationService (vector + graph + sql)
- [ ] RiskAnalyzer, SimilarityService

### Этап 5: Server — API + Hangfire (2 дня)
- [ ] Minimal API endpoints (/api/analytics/*, /api/collection/*)
- [ ] ServerAnalyticsDataService
- [ ] Hangfire jobs (сбор, refresh, embeddings)
- [ ] DI регистрация всех сервисов

### Этап 6: Client — UI (3-4 дня)
- [ ] MainLayout + навигация (MudNavMenu)
- [ ] Dashboard.razor (KPI карточки + графики)
- [ ] Estimate.razor (форма + результат)
- [ ] TaskExplorer.razor (таблица + фильтры)
- [ ] TaskDetail.razor (метрики + граф + история)
- [ ] GraphViewer.razor (визуализация графа)
- [ ] TeamStats.razor (статистика разработчиков)
- [ ] DataCollection.razor (запуск/статус сбора)
- [ ] Settings.razor (настройки)
- [ ] AnalyticsDataService (клиентская реализация через HttpClient)
- [ ] DashboardCacheService

### Этап 7: MCP Server (1 день)
- [ ] JSON-RPC stdio server
- [ ] Resources: analytics://estimate, similar, team, risk
- [ ] Markdown formatting

### Этап 8: Тесты + локализация (2 дня)
- [ ] Unit-тесты (calculators, parsers, classifier)
- [ ] Integration-тесты (Testcontainers)
- [ ] Локализация ru-RU / en-US
- [ ] Промпты для MCP Server (estimation.md, risk_assessment.md)
